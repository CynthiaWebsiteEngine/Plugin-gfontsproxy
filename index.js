...
function replacewithurlstome(body) {
    if (typeof body == "string") {
        return body.replaceAll("(http", "(/es/gfonts/http").replaceAll("(https://fonts.gstatic.com/", "(/es/gfonts/https://fonts.gstatic.com/");
    }
}
...
app.get("/*", (req, res) => {
    ...
    if (!fs.existsSync(fontpath)) {
        https.get(fonturl, (dl) => {
            ...
            writer.on("finish", () => {
                writer.close();
                res.send(replacewithurlstome(fs.readFileSync(fontpath, {encoding: "utf8", flag: "r",})));
            });
        });
    } else {
        res.send(replacewithurlstome(fs.readFileSync(fontpath, {encoding: "utf8", flag: "r",})));
